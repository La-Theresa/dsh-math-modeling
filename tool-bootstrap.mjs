 ...
 * (`dev_tool_search`, `skill_search`, `skill_load`) PLUS the math workbench
 * tool (`math_code`) plus whatever the model explicitly unlocked via
...
export const name = 'math-tool-bootstrap'
...
/** Math workbench tools always resident after promotion. */
const RESIDENT_MATH_TOOLS = ['math_code']

/** Background job tools are kept resident when the host provides them. */
const RESIDENT_BACKGROUND_TOOLS = ['job_list', 'job_output', 'job_kill']
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      if (status.promoted) {
        ...
        // discovery tools + math_code + whatever the model explicitly
        // unlocked via dev_tool_search — instead of dumping the whole
        // Standard catalog at once (the post-promotion regression fix; see
        // the header note).
        const keep = new Set([...bootstrapTools, ...RESIDENT_DISCOVERY_TOOLS, ...RESIDENT_MATH_TOOLS])
        const availableNames = new Set(assembled.tools.map((tool) => tool.name))
        if (availableNames.has('read_image')) keep.add('read_image')
        for (const toolName of RESIDENT_BACKGROUND_TOOLS) {
          if (availableNames.has(toolName)) keep.add(toolName)
        }
        for (const name of unlockedFor(context.agent?.session)) keep.add(name)
